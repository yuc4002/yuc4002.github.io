...
`ggplot2` is a graphing library in R that makes beautiful graphs. It is a R package created by R programmer Hadley Wickham. ggplot2 one of the most widely used R packages. Moreover, it builds data graphics incrementally from small pieces of code.
...
```{r eval=F}
# Basic box plot
ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot()

# Notched box plot
ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot(notch=TRUE)
```
...
# Basic box plot
p1 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot()

# Notched box plot
p2 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot(notch=TRUE)
...
# add mean points to a box plot
p2 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot()+
  stat_summary(fun.y=mean, geom="point", shape=23, size=4)
...
```{r fig.height=4, fig.width=5, message=FALSE, warning=FALSE, paged.print=FALSE}
ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot()+
  scale_x_discrete(limits=c(">0%", ">15%", ">35%"))
```
...
ggplot(data = CIACountries, aes(y=gdp, x = net_users, color=net_users)) + geom_boxplot()

# Change box plot colors by groups
ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot()
```
...
# Change box plot line colors by groups
p1 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users, color=net_users)) + geom_boxplot()

# Change box plot colors by groups
p2 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot()
...
```{r eval=F}
# Use single color
ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot(fill="steelblue", color="black")

# Use custom color palettes
ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot()+
  scale_fill_manual(values=c('#999999','#E69F00', '#56B4E9','#0072B2', '#D55E00', '#CC79A7'))
```
...
# Use single color
p1 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users)) + geom_boxplot(fill="steelblue", color="black")
...
# use brewer color palettes
p1 <-ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot() +
  scale_fill_brewer(palette="Blues") 
...
# Remove legend
p3 <- ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot()+
  theme(legend.position="none")
...
#### 4.2 Reverse the legend order

The following command can reverse the legend order :

+ `guides(fill = guide_legend(reverse=TRUE))`
+ `scale_fill_discrete(guide = guide_legend(reverse=TRUE))`

```{r eval=F}
# first menthod to reverse the legend order
ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot() +
  guides(fill = guide_legend(reverse=TRUE))

# second menthod to reverse the legend order
ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot() +
  scale_fill_discrete(guide = guide_legend(reverse=TRUE))
```
...
ggplot(data = CIACountries, aes(y=gdp, x = net_users, fill=net_users)) + geom_boxplot() +
  ...
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9","#0072B2", "#D55E00", "#CC79A7"), 
                    name="Internet use \nPercentage",
                    breaks=c(">0%", ">5%", ">25%", ">35%", ">60%", "NA"),
                    labels=c(">0%", ">5%", ">25%", ">35%", ">60%", ""))
```


#### 4.4 Modifying the legend box
...
#### 4.5 Modifying legend title and labels
...
The command `facet_wrap()` can split one graph to mutiple by one character. The following is an example of using `facet_wrap()` function.

```{r fig.height=4, message=FALSE, warning=FALSE, paged.print=FALSE}
ggplot(data = CIACountries, aes(y=gdp, x = educ)) +
geom_point() +
guides(color=guide_legend(title="Internet Use"),
size=guide_legend(title='Roadway')) +
